...
check_connection() {
	wget -q $(curl -s "https://api.github.com/repos/anddea/revanced-patches/releases/tags/v3.6.0" | jq -r '.assets[] | select(.name == "patches.json") | .browser_download_url') || rm -f "patches.json"
